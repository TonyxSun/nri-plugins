...
// to a pure shared (fractional) allocation. PrettyName == name; GetID == id.
...
func TestAllocateResourcesReclaimsStaleInstance(t *testing.T) {
	...
	old := newBurstableCtr("socialgraph/sg--hkrtv/metrics-absorber", "8a52eae-old")
	cur := newBurstableCtr("socialgraph/sg--hkrtv/metrics-absorber", "580ff89d-new")
...
// TestUpdateResourcesPathUnchanged is a structural guard: UpdateResources calls
// allocateResources directly (not AllocateResources), so releaseStaleInstances
// never runs on the update path. It passes both pre-fix and post-fix by design.
