	...
	p.memAllocator, err = libmem.NewAllocator(
		...
		libmem.WithCustomFunctions(&libmem.CustomFunctions{
			HandleOvercommit: func(overcommit map[libmem.NodeMask]int64, a libmem.CustomAllocator) error {
				// When memory pinning is disabled the policy never writes cpuset.mems
				// (see applyGrant: mems stays empty unless opt.PinMemory). The internal
				// per-zone/node memory model therefore enforces nothing, so it must not
				// gate admission. Fail open: accept the overcommit and let the kernel
				// place memory. Keep strict accounting when pinMemory is enabled.
				if !opt.PinMemory {
					log.Warn("pinMemory=false: accepting memory overcommit %v (admission not enforced)", overcommit)
					return nil
				}
